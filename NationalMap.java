...
{
    private HashMap<String, HashMap<String, County>> map;//String = statename, String = county name, county = county object
    static final int CANVAS_HEIGHT = 600;

    public NationalMap(int year) throws Exception
    {
        map = new HashMap<>();
        ...
        for(int i = 0; i < states.length; i++)
        {
            ...
            HashMap<String, County> data = new HashMap<>();
            System.out.println("\n" + states[i] + "\n");
            ...
            while(votingScanner.hasNextLine())
            {
                ...
                {
                    data.put(pollNumbers[0], new County(states[i], "RED"));
                }
                else if(demo > rep && demo > indep)
                {
                    data.put(pollNumbers[0], new County(states[i], "BLUE"));
                }
                else if(indep > rep && indep > demo)
                {
                    data.put(pollNumbers[0], new County(states[i], "GREEN"));
                }
            }
    ...
    public void processGeographicData() throws Exception
    {
        String str = "./input/USA-county.txt";
        File f = new File(str);
        ...
        for(int i = 0; i < parts; i++)
        {
            ...
            else if (map.get(stateName).get(countyName).getParty().equals("RED"))
            {
                StdDraw.setPenColor(Color.RED);
            }
            else if (map.get(stateName).get(countyName).getParty().equals("BLUE"))
            {
